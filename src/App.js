...
export class App extends React.Component {
  ...
  getCity = async (event) => {
    ...
    let cityUrl = `https://eu1.locationiq.com/v1/search.php?key=${process.env.REACT_APP_MAP_KEY}&q=${this.state.searchQuery}&format=json`;
    try {
    ...
    //  const url = `${serverRoute}/weather?city=${this.state.searchQuery}&lon=${this.state.longitude}&lat=${this.state.latitude}`;
      let importedWeatherData = await axios.get(`${process.env.REACT_APP_URL}/weather?city=${this.state.searchQuery }`);
      this.setState({
  ...
  try {
    let importedMoviesData = await axios.get(`${process.env.REACT_APP_URL}/movie?`, { params: { city: this.state.searchQuery } });
    this.setState({
